...
import com.jun.service.account.domain.exceptions.ResourceNotFoundException;
...
public class CategoryService extends BaseService {
  ...
  public PageResponse<Category> findAll(Pageable pageable) {
    List<Category> categoryList = categoryStorage.findAll();
    if (categoryList == null || categoryList.size() <= 0) {
      throw new ResourceNotFoundException("No category found!");
    }
    Page<Category> categoryPage = mapperUtil.listToPage(categoryList, pageable);
    return PageResponse.createFrom(categoryPage);
  }
